		...
		{					
				...
				if(ebBits&E_PW_CHECK)
				{
					if(!strncmp((char *)SHApp.PW , (char *)LastPw , Last_PwLength))
					{
						...
						else
						{
							...
							if (SHApp.CTemp > SHApp.ACTemp+ACTEMP_RANGE)			xEventGroupSetBits(egEventsAppliance,E_OPEN_LOCK|E_ADJUSTLIHTING|E_STARTFAN);
							else if(SHApp.CTemp < SHApp.ACTemp-ACTEMP_RANGE)	xEventGroupSetBits(egEventsAppliance,E_OPEN_LOCK|E_ADJUSTLIHTING|E_STARTHEATER);
							else                                                xEventGroupSetBits(egEventsAppliance,E_OPEN_LOCK|E_ADJUSTLIHTING|E_STOPHEATER|E_STOPFAN);
						}			
						ErrorCounter = 0;
			...
			else if(SS_MAIN == SHApp.SysState)
			{
				...
				if(Last_Light_Mode != SHApp.Light_Mode)
				{
					...
					xEventGroupSetBits(egEvents,E_MAIN_SCREEN);
